...
api/         → FastAPI service (`app.py` done, `Dockerfile` pending)
...
**Current priority (as of 2026-03-29):** Step 8 FastAPI part is done; finish Docker + README polish + GitHub-ready packaging, then submit resume V1 by April 7. See `docs/tasks/YOLO 0327-0409.md` for the daily execution plan.
...
/d/Base/Tools/Anaconda/Anaconda3/envs/yolo_defect/python.exe scripts/benchmark_api.py
```
...
### Step 8: FastAPI Service — In Progress (2026-03-29)
- Implemented `api/app.py` with two endpoints:
  - `GET /health` → returns service/model status
  - `POST /detect` → accepts image upload and returns JSON detections
- Model loads once at startup and reuses `src/detector.py`
- Added basic API engineering features:
  - file/validation error handling
  - request logging
  - response-time header `X-Response-Time-MS`
  - `/health` request statistics
- Local API test passed:
  - `/health` returned `200 OK` with `status=ok`
  - `/detect` on `data/images/val/crazing_241.jpg` returned `count=3`
- Added `scripts/benchmark_api.py` for simple concurrent benchmark
- Local benchmark result (10 images, concurrency 10):
  - success requests: `10/10`
  - average client-observed response time: `2333.37 ms`
  - total wall time: `3.06 s`
  - throughput: `3.27 QPS`
  - most service-side `inference_time_ms`: `13-23 ms`
  - first request cold-start / queueing spike: `2198.88 ms`
