...
### Experiment 1 (imgsz=512)

| Class | AP@0.5 | Precision | Recall |
|-------|--------|-----------|--------|
| inclusion | 0.835 | 0.732 | 0.774 |
| patches | 0.909 | 0.798 | 0.860 |
| pitted_surface | 0.821 | 0.770 | 0.759 |
| scratches | 0.821 | 0.581 | 0.868 |
| rolled-in_scale | 0.520 | 0.490 | 0.492 |
| crazing | 0.476 | 0.585 | 0.235 |

### Comparison Insight

- `imgsz=512` reduced training time from about 9 minutes to about 7 minutes
- Overall mAP stayed close to baseline, but the two hardest classes degraded:
  - `crazing`: `0.542 -> 0.476`
  - `rolled-in_scale`: `0.581 -> 0.520`
- Practical conclusion: smaller input is faster, but not a better choice for this project because it weakens fine-texture defect detection
...
```bash
python scripts/export_onnx.py --weights runs/detect/train2/weights/best.pt
...
│   └── exp1.yaml                 # Experiment 1 config (imgsz=512)
├── models/
...
![PR Curve Exp1](docs/assets/PR_curve_exp1.png)
...
![Confusion Matrix Exp1](docs/assets/confusion_matrix_exp1.png)

### 预测样例

![Validation Predictions](docs/assets/val_pred_sample.jpg)

![Validation Predictions Exp1](docs/assets/val_pred_sample_exp1.jpg)

### 实验结论

- `imgsz=512` 将训练时间从约 9 分钟缩短到约 7 分钟
- overall mAP 与 baseline 接近，但 hardest classes 明显退化：
  - `crazing`: `0.542 -> 0.476`
  - `rolled-in_scale`: `0.581 -> 0.520`
- 因此，这组实验说明更小输入尺寸虽然更快，但不适合当前项目的细纹理工业缺陷场景
