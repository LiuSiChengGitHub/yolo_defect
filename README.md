...
- **Resume-Ready Result** — Best checkpoint `final_train_2` reaches **mAP@0.5 = 0.743** on NEU-DET
...
Current Docker verification (2026-03-30):
...
- **可写进简历的结果** — 当前最佳模型 `final_train_2` 达到 **mAP@0.5 = 0.743**
- **ONNX 推理已实测** — ONNX Runtime 达到 **69.8 FPS（GPU）**、**22.0 FPS（CPU）**
- **服务吞吐量有实测数据** — 本地 FastAPI 压测达到 **3.27 QPS**
...
|------|---------|-----------|-----------|-----------|----------|------|
| PyTorch (.pt) | **0.7433** | **0.3880** | 7.1 | 60.5 | ~6.0 MiB (`best.pt`) | ultralytics `model.val()` |
| ONNX (.onnx) | **≈0.743** | **≈0.388** | 22.0 | **69.8** | ~11.8 MiB | `detector.py` + `onnxruntime` |
...
- 当前本地文件大小：`best.pt = 6,286,072 bytes`，`best.onnx = 12,336,935 bytes`
...
当前 Docker 本地实测（2026-03-30）：

- `docker build` 成功完成，总耗时 `184.7 s`
- `GET /health` 返回 `status=ok`、`model=best.onnx`
- `POST /detect` 对 `crazing_241.jpg` 返回 `count=3`
- 本次容器内测试返回 `inference_time_ms = 73.99`
